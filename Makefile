...
quickstart-dev:
		docker build -f .docker/Dockerfile-build -t oryd/kratos:custom-image .
...
.PHONY: docker
docker:
		DOCKER_BUILDKIT=1 docker build -f .docker/Dockerfile-build --build-arg=COMMIT=$(VCS_REF) --build-arg=BUILD_DATE=$(BUILD_DATE) -t oryd/kratos:email-final .
